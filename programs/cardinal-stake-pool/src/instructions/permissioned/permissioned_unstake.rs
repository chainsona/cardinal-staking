...
#[derive(Accounts)]
pub struct PermissionedUnstakeCtx<'info> {
    #[account(mut)]
...
pub fn handler(ctx: Context<PermissionedUnstakeCtx>) -> Result<()> {
    ...
    let cpi_accounts = cardinal_token_manager::cpi::accounts::SendCtx {
        ...
        recipient: stake_entry.to_account_info(),
        recipient_token_account: ctx.accounts.stake_entry_original_mint_token_account.to_account_info(),
        target: ctx.accounts.user.to_account_info(),
        target_token_account: ctx.accounts.user_original_mint_token_account.to_account_info(),
    ...
    let cpi_context = CpiContext::new(ctx.accounts.token_manager.to_account_info(), cpi_accounts).with_signer(stake_entry_signer);
