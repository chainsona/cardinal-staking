...
pub fn handler(ctx: Context<UnstakeCCSCtx>) -> Result<()> {
    ...
    let seed = get_stake_seed(ctx.accounts.mint.supply, user);
    ...
    // remove in_use_by
    invoke_signed(
        &remove_in_use_by(ctx.accounts.cardinal_creator_standard.key(), ctx.accounts.mint_manager.key(), stake_entry.key())?,
        &[ctx.accounts.mint_manager.to_account_info(), stake_entry.to_account_info()],
        stake_entry_signer,
    )?;
